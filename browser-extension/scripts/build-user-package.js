 ...
 *   └── grrab-prod.zip    (the actual Chrome extension to load)
 *
 * Steps:
 *   1. Run `npm run build:prod` to refresh dist/grrab-prod.zip.
...
const EXTENSION_ZIP = path.join(DIST_DIR, 'grrab-prod.zip');
...
const WRAPPER_BASENAME = `grrab-v${VERSION}-prod`;
const WRAPPER_ZIP = path.join(DIST_DIR, `${WRAPPER_BASENAME}.zip`);

// ---------- step 1: refresh grrab-prod.zip ----------
...
output.on('close', () => {
  ...
  console.log(`  - grrab-prod.zip (the extension)`);
});
...
archive.file(EXTENSION_ZIP, { name: 'grrab-prod.zip' });
