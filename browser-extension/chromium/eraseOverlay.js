...
const KC_ERASE_PLACEHOLDER_W = 320;
const KC_ERASE_PLACEHOLDER_H = 240;
 ...
 */
export function showEraseOverlay(blob, inpaintFn, commitFn, bindStatus) {
  return new Promise((resolve) => {
    ...
    const iconTextBtnStyle = btnStyle + ';display:inline-flex;align-items:center;gap:6px';
    ...
    btnCancel.style.cssText = btnStyle + ';padding:8px 6px;';
    ...
    rightGroup.append(btnCancel);
    toolbar.append(leftGroup, rightGroup);
    bottomBar.append(btnUndo, btnDone, btnRefresh);
  ...
  `;
    ...
    root.addEventListener('pointerdown', (e) => {
      if (settled || loading || busy || drawing) return;
      if (!hasSelection()) return;
      if (stage.contains(e.target)) return;
      if (toolbar.contains(e.target)) return;
      if (statusEl.contains(e.target)) return;
    ...
    const allButtons = [btnRefresh, btnUndo, btnCancel, btnDone, btnModeBrush, btnModeRect];
    ...
    function updateUi() {
      ...
      if (!busy) {
        if (hasSelection()) {
          setDoneButton(KC_ICON_ERASE, KC_REMOVE_LABEL);
        } else {
          setDoneButton(KC_ICON_CLIP, KC_CLIP_LABEL);
        }
      }
    ...
    function setRemoveBtnBusy(isBusy) {
      if (isBusy) {
        setDoneButton(KC_ICON_ERASE, KC_REMOVING_LABEL);
    ...
    async function undoLast() {
      ...
      activeStroke = null;
      try {
        await loadBlobIntoImage(current);
        if (settled) return;
        renderSelections();
      } catch (_) {
        if (settled) return;
    ...
    function layoutImage() {
      if (loading) {
        stage.style.width = `${KC_ERASE_PLACEHOLDER_W}px`;
        stage.style.height = `${KC_ERASE_PLACEHOLDER_H}px`;
        toolbar.style.width = `${KC_ERASE_PLACEHOLDER_W}px`;
        bottomBar.style.width = `${KC_ERASE_PLACEHOLDER_W}px`;
        return;
      }
      ...
      toolbar.style.width = `${dispW}px`;
      bottomBar.style.width = `${dispW}px`;
      img.style.width = `${dispW}px`;
      img.style.height = `${dispH}px`;
      strokeCanvas.style.width = `${dispW}px`;
      strokeCanvas.style.height = `${dispH}px`;
      strokeCanvas.width = dispW;
      strokeCanvas.height = dispH;
      renderSelections();
    }
    ...
    function pointerToSource(e) {
      ...
      return {
        x: Math.max(0, Math.min(Math.round(pt.x / scale), srcW)),
        y: Math.max(0, Math.min(Math.round(pt.y / scale), srcH)),
      };
    }
